...
namespace sict {
	...
	//----------------------------------------------------------------
	Contact::Contact(const Contact &pSource) {  //copy constructor

		//empty contact
		if (pSource.isEmpty() == true) {
			return;
		}
		this->copyValues(pSource);
			
	}
	//----------------------------------------------------------------
	Contact& Contact::operator= (const Contact& pSource) { // assignment operator

		// check for empty and self-assigned
		if (!isEmpty() && this != &pSource) { //if you are making object1 = object1 this self-assign ERROR
			 this->copyValues(pSource);
			}
		return *this;
	}
	//----------------------------------------------------------------
	void Contact::copyValues(const Contact& pSource) {

		strcpy(this->_name, pSource._name);
		this->_phoneQuant = pSource._phoneQuant;
		//resizes the phone array
		this->_phoneNumbers = new long long[this->_phoneQuant];
		//copies the phones
		for (int i = 0; i < this->_phoneQuant; i++) {
			this->_phoneNumbers[i] = pSource._phoneNumbers[i];
		}
	}
	//----------------------------------------------------------------
	Contact& Contact::operator+= (const long long& pNumber) { //adds new phones

		if (isValidPhone(pNumber)) { 
			//copy original phones to temporary array
			long long* locPhone = new long long[this->_phoneQuant];
			for (int i = 0; i < this->_phoneQuant; i++) {
				locPhone[i] = this->_phoneNumbers[i];
			}
			//resizes the original array
			this->_phoneNumbers = new long long[this->_phoneQuant+1];
			//copy back the phones
			for (int i = 0; i < this->_phoneQuant; i++) {
				this->_phoneNumbers[i] = locPhone[i];
			}
			//add new phone
			this->_phoneNumbers[this->_phoneQuant] = pNumber;
			this->_phoneQuant++;
	...
	void Contact::setAll(const char * pName, const long long * pPhones, const int phoneQuant) {
		...
		//resizes the phone array
		this->_phoneNumbers = new (nothrow) long long[validPhonesQuant];
